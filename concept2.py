...
def feature_and_basis(temp_image):
    ...
    features = np.vstack([expected_d, variance_d, num_zeros, 
                        expected_a_d, variance_a_d]).T

    #### gaussian basis function ####
    basis_fea_set = np.zeros((features.shape[0], 5*num_basis + 1))

    # Gaussian Basis Function을 적용하여 새로운 feature 생성

    mus = np.linspace(0, 1, num_basis)
    sigmas = np.linspace(0.1, 0.5, num_basis)

    basis_fea_set[:, 0] = 1  # bias 추가
    for i in range(features.shape[0]):
        k = 1  # bias를 제외한 feature부터 시작
        for j in range(len(mus)):
            for l in range(len(sigmas)):
                basis_fea_set[i, k] = gaussian_basis_function(features[i, j], mus[j], sigmas[l])
                k += 1
